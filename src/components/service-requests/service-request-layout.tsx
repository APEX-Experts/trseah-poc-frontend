...
import { CheckCircle2, Building2, BookOpen, ClipboardCheck } from "lucide-react";
import { ReactNode } from "react";
import { useTranslations } from "next-intl";

interface ServiceRequestLayoutProps {
  children: ReactNode;
  currentStep: number;
}

export function ServiceRequestLayout({ children, currentStep }: ServiceRequestLayoutProps) {
  const t = useTranslations("ServiceRequests");
  const tOnboarding = useTranslations("Onboarding");

  const steps = [
    { id: 1, label: t("step1"), icon: Building2 },
    { id: 2, label: t("step2"), icon: BookOpen },
    { id: 3, label: t("step3"), icon: ClipboardCheck },
  ];
  ...
  return (
    <div className="h-[calc(100vh-65px)] flex flex-col bg-background overflow-hidden relative">
      {/* Step Header */}
      <div className="bg-white border-b border-border py-6 px-4 md:px-10 flex items-center justify-between sticky top-0 z-30 shadow-sm">
        <div className="flex w-full max-w-3xl mx-auto items-center justify-between relative">
          {steps.map((step) => {
              ...
              >
                ...
                <span
                  className={`text-xs md:text-sm font-medium hidden md:block ${
                    status === "upcoming" ? "text-neutral-400" : "text-primary-800"
                  }`}
                >
                  {step.label}
                </span>
        ...
        <aside className="hidden lg:flex w-[280px] shrink-0 bg-white border-s border-border p-8 flex-col">
          <div className="mb-8">
            <h2 className="text-xl font-bold text-primary-900 mb-2">
              {tOnboarding("Actions.progress", { fallback: "Progress" })}
            </h2>
            <div className="flex items-center justify-between text-sm text-neutral-500 mb-3">
              <span>
                {tOnboarding("progress", { currentStep, maxSteps: steps.length })} —{" "}
      ...
      {/* Portal destination for the footer actions */}
      <div
        id="service-request-footer"
        className="absolute bottom-0 inset-x-0 bg-white border-t border-border px-10 py-4 shadow-[0_-4px_20px_rgba(0,0,0,0.02)] z-10"
      />
    </div>
  );
}
